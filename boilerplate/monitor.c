...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, link) {
        ...
        if (!entry->soft_reported && rss_bytes > (long)entry->soft_limit_bytes) {
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        mutex_lock(&monitored_lock);
        list_for_each_entry(cur, &monitored_list, link) {
            if (cur->pid == entry->pid &&
                strncmp(cur->container_id, entry->container_id, sizeof(cur->container_id)) == 0) {
                cur->soft_limit_bytes = entry->soft_limit_bytes;
                cur->hard_limit_bytes = entry->hard_limit_bytes;
                cur->soft_reported = false;
                mutex_unlock(&monitored_lock);
                kfree(entry);
                return 0;
            }
    ...
    mutex_lock(&monitored_lock);
    {
        ...
        list_for_each_entry_safe(cur, tmp, &monitored_list, link) {
            if (cur->pid == req.pid &&
